...
from axial_attention import AxialAttention #Axial Attention implemented by: 
...
parser.add_argument('--att_layers', type=int, default=2, help='Number of attention-layers')
...
if opt.version == 2:
    ...
    class mydata(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            #Creating a one-hot-encoding tensor of size (6,256,256) containing zeros everywhere except at matrix i=randint
            #Then repeating (input_seq+6) times to get tensor ((input_seq+6),6,256,256) so a one-hot encoding for each input+output frame
            #Then permuting the tensor to get the shape (6,(input_seq+6), 256,256) so we can concat with data of form (C,S,W,H) -> gettting shape (C+6,S,W,H)
            times = torch.eye(6)[randint].unsqueeze(-1).unsqueeze(-1)
            ones = torch.ones(1, 256,256)
            lead_time = ((times*ones).repeat(10,1,1,1)).permute(1,0,2,3)
...
# Defining ConvLSTM encoder (temporal encoder)
class EncoderConvLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, frame_size, input_seq):
    ...
    def forward(self, x, hidden_state=None):
        ...
        # autoencoder forward
        output = self.autoencoder(x, h_t, c_t, h_t2, c_t2)
...
class SpatialAggregator(nn.Module):
    def __init__(self, dim, dim_index, heads = 4, num_dimensions = 2, n_att_layers = 2):
        ...
        #                            )
        self.spatial_agg = nn.Sequential(*[AxialAttention(dim=dim, dim_index=dim_index, heads=8, num_dimensions=2) for _ in range(n_att_layers)])
...
class MetNet(pl.LightningModule):
    def __init__(self, input_dim, hidden_dim, kernel_size, frame_size, input_seq, lr, batch_size, n_att_layers):
        ...
        self.spatialdown = spatialdownsampling(input_dim = input_dim)
        self.feature = featureextractor(input_dim = input_dim)
        self.temporal = EncoderConvLSTM(input_dim = input_dim, hidden_dim = hidden_dim, kernel_size = kernel_size, frame_size = frame_size, input_seq = input_seq)
        self.spatial = SpatialAggregator(dim = hidden_dim, dim_index = 1, heads = 4, num_dimensions = 2, n_att_layers = n_att_layers)
    ...
    def forward(self, x):
        ...
        # Then 3d conv to get right output shape
    ...
    def create_grid(self, output, target):
        ...
        grid = torchvision.utils.make_grid([final[i] for i in range(final.shape[0])], nrow=1)
    ...
    def configure_optimizers(self):
        ...
        optimizer = AdamW(self.parameters(), lr=self.lr, betas=(opt.beta_1, opt.beta_2), weight_decay=opt.weight_decay)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience = 3, factor = opt.factor)
    ...
    def test_epoch_end(self, outputs):
        ...
        all_targets = torch.cat([x["target"] for x in outputs], dim=0).flatten().cpu().numpy().astype(int)
        all_output = torch.cat([x["output"] for x in outputs], dim=0).flatten().cpu().numpy()
                #Creating ROC-curve
        ns_probs = [0 for _ in range(len(all_targets))]
        
        ns_auc = roc_auc_score(all_targets, ns_probs)
        lr_auc = roc_auc_score(all_targets, all_output)
        
        
        # calculate roc curves
        ns_fpr, ns_tpr, _ = roc_curve(all_targets, ns_probs)
        lr_fpr, lr_tpr, _ = roc_curve(all_targets, all_output)
        #saving
        np.save(opt.model_name+"/"+opt.model_description+"/savedTensors/ns_fpr.npy", ns_fpr)
        np.save(opt.model_name+"/"+opt.model_description+"/savedTensors/ns_tpr.npy", ns_tpr)
        np.save(opt.model_name+"/"+opt.model_description+"/savedTensors/lr_fpr.npy", lr_fpr)
        np.save(opt.model_name+"/"+opt.model_description+"/savedTensors/lr_tpr.npy", lr_tpr)
...
def run_trainer():
    if not opt.testing:
        ...
        if opt.continue_from_latest:
            ...
            trainer = Trainer(fast_dev_run = False, gpus=opt.n_gpus, max_epochs = opt.epochs, logger=logger, callbacks = [epoch_callback,lr_monitor], resume_from_checkpoint=c_path, num_sanity_val_steps=0, accumulate_grad_batches=opt.gradbatches)
            model = MetNet.load_from_checkpoint(c_path)
        else:
            trainer = Trainer(fast_dev_run = False, gpus=opt.n_gpus, max_epochs = opt.epochs, logger=logger, callbacks = [epoch_callback,lr_monitor], auto_lr_find = opt.auto_lr, num_sanity_val_steps=0, accumulate_grad_batches=opt.gradbatches)
            model = MetNet(opt.input_dim, opt.n_hidden_dim, opt.kernel_size, opt.frame_size, opt.input_seq, opt.lr, opt.batch_size, opt.att_layers)
        ...
        latest_checkpoint = sorted_checkpoints[-1]
        c_path = opt.model_name+"/"+opt.model_description+"/savedCheckpoints/"+latest_checkpoint
        model_test = MetNet.load_from_checkpoint(c_path)
