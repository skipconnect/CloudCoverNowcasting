...
class UNet(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch_idx == best_idx:
            torch.save(target,opt.model_name+"/"+opt.model_description+"/BestTarget.pt")
            torch.save(torch.sigmoid(output),opt.model_name+"/"+opt.model_description+"/BestOutput.pt")
            torch.save(mse_loss,opt.model_name+"/"+opt.model_description+"/BestMSE.pt")
        if batch_idx == worst_idx:
            torch.save(target,opt.model_name+"/"+opt.model_description+"/WorstTarget.pt")
            torch.save(torch.sigmoid(output),opt.model_name+"/"+opt.model_description+"/WorstOutput.pt")
            torch.save(mse_loss,opt.model_name+"/"+opt.model_description+"/WorstMSE.pt")
        if opt.version == 1:
            #MSE_Loss pr. frame
