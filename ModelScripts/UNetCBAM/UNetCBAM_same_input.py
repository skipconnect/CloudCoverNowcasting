...
if opt.version == 1: #(With all 6 frames at once)
    class mydata(Dataset):
        ...
        def __getitem__(self, idx):
            batch = self.data[idx]
            input_ = batch[:,:self.input_seq,64:192, 64:192]
            target = batch[0,self.input_seq:,64:192, 64:192]
            #Reshaping input-size by collapsing channels,input_seq to one dimension of channels*input_seq (concatanating)
...
if opt.version == 2: #(With lead-time)
    ...
    class mydata(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            input_ = final[:,0:self.input_seq,64:192, 64:192]
...
class StackEncoder(nn.Module):
    def __init__(self, x_channels, y_channels, kernel_size=(3, 3), first=False):
...
class UNet(pl.LightningModule):
    def __init__(self, input_dim, kernel_size, input_seq, output_seq, lr, batch_size):
        ...
        self.down1 = StackEncoder(input_dim*input_seq, 24, kernel_size=kernel_size, first = True)  # 128 (input = 128)
