...
class EncoderDecoderConvLSTM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = F.l1_loss(torch.sigmoid(output).flatten(), target.flatten())
    ...
    def test_step(self, batch, batch_idx):
        ...
        PSNR = sum([20*math.log10(1/math.sqrt(F.mse_loss(torch.sigmoid(output[i,:,j]).flatten(),target[i,:,j].flatten()).detach().item()+0.000001)) for i in range(target.shape[0]) for j in range(target.shape[2])])
    ...
    def test_epoch_end(self, outputs):
        ...
        BCEs = torch.cat([x["loss1"].cpu() for x in outputs],0)
        loss1_median = torch.median(BCEs.flatten())
        
        
        #maximums = [torch.max(x["loss1"].cpu()) for x in outputs]
        print("All:",BCEs.shape)
        print("Single:",BCEs[0,].shape)
        torch.save((BCEs[0]),opt.model_name+"/"+opt.model_description+"/savedMseTensors/BCE_0.pt")
        #plt.hist(BCEs[0].flatten())
        #plt.savefig("HISTOGRAM.png")
        #plt.close()
        #print("Median:", loss1_median)
        total_mse_loss = torch.stack([x["MSE_test_loss"] for x in outputs]).sum()
        total_datapoints = sum([x["data_points"] for x in outputs])
        #test = outputs[0]["output"]
        #print("min:", torch.min(torch.sigmoid(test)))
        #print("max:", torch.max(torch.sigmoid(test)))
        #print("mean:", torch.mean(torch.sigmoid(test)))
