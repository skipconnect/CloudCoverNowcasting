...
class spatialdownsampling(nn.Module):
    def __init__(self, input_dim):
        ...
        self.conv1 = nn.Conv3d(1, 32, kernel_size=(1,3,3), padding=(0,1,1)) #256
...
class EncoderDecoderConvLSTM(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        #False-positive plot (vi tror der er en sky, men det er der ikke)
        
        #False-Negative plot (vi tror IKKE der er en sky, men det er der)
        out1 = output.detach().clone()
        target1 = target.detach().clone()
        #Binarize
        out1[out1 < 0.5] = 0
        out1[out1 >= 0.5] = 1
        target1[target1 < 0.5] = 0
        target1[target1 >= 0.5] = 1
        
        fn =out1-target1
        fn[fn>-0.5]=0
        fn[fn<-0.5]=1 #this is FN
        
        fp=target1-out1
        fp[fp>-0.5]=0
        fp[fp<-0.5]=1 #this is FP
        ...
        if opt.version == 1:
            ...
            #MSE for the current lead
            mse_loss_full_lead = F.mse_loss(torch.sigmoid(output), target, reduction = "none")
        ...
        return {"MSE_test_loss": mse_loss, 
                ...
                "MSE_test_loss_full_lead": mse_loss_full_lead if opt.version == 2 else 0,
    ...
    def test_epoch_end(self, outputs):
        ...
        #False Negatives
        Fns = torch.cat([x["FN"] for x in outputs],0).sum(dim = 0)
        torch.save((Fns/total_datapoints).cpu(),opt.model_name+"/"+opt.model_description+"/savedTensors/FN.pt")
        #False Positives
        Fps = torch.cat([x["FP"] for x in outputs],0).sum(dim = 0)
        torch.save((Fps/total_datapoints).cpu(),opt.model_name+"/"+opt.model_description+"/savedTensors/FP.pt")
        ...
        if opt.version == 1:
            ...
            #MSE for the current lead
            total_mse_loss_full_lead = torch.cat([x["MSE_test_loss_full_lead"] for x in outputs],0).sum(dim = 0)
            torch.save((total_mse_loss_full_lead / total_datapoints),opt.model_name+"/"+opt.model_description+"/savedMseTensors/"+str(opt.leadtime)+".pt")
